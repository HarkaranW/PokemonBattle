...
export default class Pokemon extends GameEntity {
	...
	constructor(name, definition, level) {
		...
		// Load moves
		this.moves = [];
		if (definition.starterMoves && definition.movesData) {
			definition.starterMoves.forEach(moveName => {
				const moveDefinition = definition.movesData[moveName];
				if (moveDefinition) {
					this.moves.push(new Move(moveName, moveDefinition));
				}
			});
		}
	...
	inflictDamage(defender, move) {
		...
		// Return the move type for effectiveness calculation
		return move ? move.type : 'Normal';
	}
